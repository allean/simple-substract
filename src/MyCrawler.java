...
public class MyCrawler extends WebCrawler {
	...
	public boolean shouldVisit(WebURL url) {
		...
		return !FILTERS.matcher(href).matches()
				&& href.startsWith("http://shipei.qq.com/");
	}
	...
	public void visit(Page page) {
		...
		if (page.getParseData() instanceof HtmlParseData) {
			...
			if (url.indexOf("news") > 0 || url.indexOf("sports") > 0
					|| url.indexOf("ent") > 0 || url.indexOf("lady") > 0
					|| url.indexOf("games") > 0 || url.indexOf("house") > 0
					|| url.indexOf("astro") > 0) {
				String content = html;
				...
				write("./" + url.substring(url.lastIndexOf("/")) + ".txt",
						result);
			}
	...
	private static boolean isUseFull(Term term) {
		String nstr = term.getNatrue().natureStr;
		return !unUsefulWord.contains(term.getName()) && ("n".equals(nstr));
	}
